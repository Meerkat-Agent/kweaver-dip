...
import {
  ...
  getAllTaskNode
} from '@/pages/Benchmark/BenchmarkConfig/BenchmarkConfigGraph/ConfigGraph/assistant';
          ...
          onChange={checked => {
            ...
            if (checked) {
              ...
              averageTaskId = allTaskNode
                .filter(taskNode => {
                  const datasetNodeForTask = allDatasetMetricNode.filter(
                    node => node.getData().dataset.taskNodeId === taskNode.id
                  );
      ...
      <>
        {datasetMetricNodes.length > 1 && (
          <div style={{ minHeight: 45 }} className={classNames('ad-flex ad-align-center ad-border-b ad-pl-3 ad-pr-3')}>
            ...
            <div
              style={{ minHeight: 45 }}
              className="ad-flex-item-full-width ad-align-center ad-border-l ad-h-100 ad-pt-3 ad-pb-3"
            >
  ...
  const dataSource = [
    {
      ...
      value: (
        <div className="ad-c-text-lower ad-align-center ad-pl-3 ad-pr-3 ad-h-100">
          {intl.get('benchmark.config.displayParam')}
        </div>
      )
    },
    {
    ...
    },
    {
      label: (
        <span className="ad-align-center">
          {intl.get('benchmark.config.divideEqually')}
          <Tooltip
            className="ad-ml-1 ad-c-subtext"
            title={intl.get('benchmark.config.totalAverageTip')}
            placement="top"
          >
