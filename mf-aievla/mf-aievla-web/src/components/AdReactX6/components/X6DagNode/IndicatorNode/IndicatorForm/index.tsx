...
const IndicatorForm = ({ node, editData, visible }: any) => {
  ...
  const checkboxChange = (keys: any) => {
    ...
    const curHasKeys = metricData.metricIdList.filter(
      (key: string) => keys.includes(key) || AiDataRef.current.includes(key)
    );
          ...
          <div className="ad-space-between">
            ...
            {!readOnly && (
              <span
                ...
                className={`${
                  metricData.metricIdList.length === 0
                    ? 'ad-c-watermark ad-not-allowed'
                    : 'ad-c-text-lower ad-c-hover-color-deepens ad-pointer'
            ...
            <div className="ad-flex-item-full-height ad-flex-column">
              <div className="ad-mt-4 ad-flex-item-full-height">
                <ADTable
                  ...
                  rowClassName={record =>
                    indicatorData?.id === record.id ? 'IndicatorForm-selected-row ad-pointer' : 'ad-pointer'
                  }
