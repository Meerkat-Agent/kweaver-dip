...
import {
  getAlgorithm,
  onHandleCommonFormat
} from '@/pages/Benchmark/BenchmarkTasks/CreateBenchmarkTask/assistFunction';
import {
  onHandleParentAndChildrenKeys,
  onHandleAllCheckSwitchOperate,
  onHandleChildrenChecked,
  onHandleInitializeCheckData
} from './assistFunction';
...
const RunModal = (props: any) => {
  ...
  const onCheck = (checkedKeysArr: any, info: any) => {
    ...
    const updateChecked = checked
      ? [...checkedKeys, ...filterCheckedKeysArr]
      : _.filter(checkedKeys, (item: any) => !_.includes(filterCheckedKeysArr, item));
      ...
      <>
        {`${key}`?.split('*')?.[1] ? (
          ...
          <div className="ad-align-center ad-mt-4">
            <Input
              className={classNames({ 'ad-mr-3': isLargeModalExternal })}
              style={{ width: isLargeModalExternal ? 262 : 676 }}
              value={title}
              disabled
            />
            {algorithmType === 1 ? (
