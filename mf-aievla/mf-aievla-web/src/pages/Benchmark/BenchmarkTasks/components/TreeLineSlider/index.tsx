...
const TreeLineSlider = (props: any, ref: any) => {
  ...
  const addBtnIsShow =
    disabled || (selectedValueArr?.length && selectedValueArr?.length === allListData?.length) || isSelectedEmpty;
      ...
      <div title="">
        {`${key}`?.split('*')?.[1] ? (
          <div className="ad-align-center ad-mt-4">
            ...
            <Tooltip
              title={intl.get(`benchmarkTask.${is_run ? 'closeRun' : 'startRun'}`)}
              placement="top"
            ...
            >
              <Switch
                ...
                checked={is_run}
                disabled={disabled}
                onChange={(checked: boolean) => onSwitchChange(checked, key)}
              />
            </Tooltip>
                  ...
                  {loading ? (
                    ...
                    _.map(optionalList, (item: any) => {
                      return (
                        ...
                        >
                          <div
                            className="ad-align-center ad-space-between"
                            title={isAgentType ? item.name : item?.[TYPE_TO_NAME[algorithm_type]]}
                          >
