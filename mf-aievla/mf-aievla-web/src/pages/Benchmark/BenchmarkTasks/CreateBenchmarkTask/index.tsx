...
import {
  ...
  COMMON_HANDLE_TYPE,
  AGENT
} from '@/pages/Benchmark/BenchmarkTasks/enums';
...
const CreateBenchmarkTask = () => {
  ...
  const beforeUpload = (file: any) => {
    ...
    if (name?.[name?.length - 1] !== 'py' || !isSizeValid) {
      setIsError('');
      message.warning(
        intl.get(`${!isSizeValid ? 'benchmarkTask.fileSizeError' : 'benchmark.indicator.fileFormatErrorTip'}`)
      );
      return false;
    }
    return true;
  ...
  const onCreateAndUpdate = async (values: any, type: 'save' | 'run' | 'saveCancel') => {
    try {
      ...
      const callCondition = isActionCopy
        ? benchmarkCopyTaskId.current === benchmarkTaskId.current
        : benchmarkTaskId.current;
          ...
          if (res) {
            if (!_.isEmpty(dataCheck)) {
              knowModalFunc.open({
                ...
                content: intl.get(
                  `global.${body.algorithm.length > 1 ? 'runSuccessSomeButLimit' : 'runSuccessButLimit'}`
                ),
  ...
  const onRun = (allValues: any) => {
    return new Promise(async resolve => {
      try {
        const benchmarkConfigIsPublished = _.filter(
          _.cloneDeep(benConfigList),
          (item: any) => item?.id === allValues?.config_id
        );
  ...
  const onHandleError = (err: any) => {
    ...
    if (
      _.includes(
        ['ADTask.BenchmarkTask.AddBenchmarkTask.NameExist', 'ADTask.BenchmarkTask.EditBenchmarkTask.NameExist'],
        ErrorCode
      )
    ) {
    ...
    <div className="ad-flex errorBox ad-mb-6">
      <div className="ad-flex-column">
        ...
        <div className="ad-content-center">
          {isError === 'error' ? intl.get('global.fileFailed') : intl.get('benchmarkTask.noData')}
        </div>
      </div>
    </div>
  );
    ...
    <>
      <span className="ad-mr-6">
        {currentAction === 'create'
          ? intl.get('benchmarkTask.createTask')
          : currentAction === 'copy'
            ? `${taskInfo?.name}的副本`
            : taskInfo?.name}
      </span>
  ...
  return (
    <div className="createBenchmarkTask ad-flex-column">
      {isRunLoading || isEditViewLoading || isAdapterLoading ? (
        <div
          className={classNames('loading-mask', { spinning: isRunLoading || isEditViewLoading || isAdapterLoading })}
        >
          <div className="spin-content-box ad-content-center ad-flex-column">
            <AdSpin />
            {isEditViewLoading || isAdapterLoading ? null : (
              <div className="ad-mt-3">{intl.get('global.loadingDot')}</div>
            )}
          </div>
        </div>
      ) : null}
                      ...
                      !isView ? (
                        <div>
                          ...
                          <div className="list">
                            ...
                            <span
                              className="ad-pointer ad-c-primary"
