...
export const onHandlePromptItemTypes = (data: any) => {
  const result = _.reduce(
    _.cloneDeep(data),
    (pre: any, key: any) => {
      const newPromptItemTypes =
        _.map(_.cloneDeep(pre?.[key.prompt_item_id]?.prompt_item_types), (item: any) => item.id) || [];
      ...
      if (!_.includes(newPromptItemTypes, key.prompt_item_type_id)) {
        allPromptItemTypes = [
          { name: key.prompt_item_type, id: key.prompt_item_type_id },
          ...(pre?.[key.prompt_item_id]?.prompt_item_types || [])
        ];
      }
