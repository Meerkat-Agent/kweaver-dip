...
const AddPromptModal = ({ onClose, onAddPrompt, treeNode, disabled }: any) => {
  ...
  const onHandlePromptListTreeData = (data: any) => {
    return _.map(data, item => {
      return {
        ...
        title: (
          <div
            className="ad-ellipsis ad-w-100"
            style={{ maxWidth: 217, width: 217 }}
    ...
    return _.map(source, item => {
      ...
      return {
        ...
        title: (
          <div
            className="ad-ellipsis"
            style={{ maxWidth: 243, width: 243 }}
            title={prompt_item_name}
            onClick={() => onTreeExpand([`${prompt_item_id}`], true)}
          >
            {prompt_item_name}
          </div>
        ),
  ...
  const onTreeExpand = (keys: any[], isPrevent = false) => {
    if (isPrevent && !_.isEmpty(expandKeysRef?.current)) {
      ...
      const expandKeys = isClose
        ? _.filter(_.cloneDeep(expandKeysRef?.current), (item: any) => item !== `${keys?.[0]}`)
