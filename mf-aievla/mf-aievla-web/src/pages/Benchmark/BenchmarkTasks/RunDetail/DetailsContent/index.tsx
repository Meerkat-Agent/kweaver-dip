...
const DetailsContent = (props: any) => {
  ...
  const onViewLog = (data: any, dataIndex: any) => {
    ...
    window.open(
      `${prefix}/effect-evaluation/log?id=${detailData?.record?.id}&algorithm_id=${data?.algorithm_id}&config_task_id=${dataIndex}`
    );
  };
  ...
  const onResetRun = async (tableRecord: any, dataIndex: any, isStopRestart = false) => {
    ...
    const newAlgorithmData = _.map(_.cloneDeep(algorithmData), (item: any) => {
      const algorithmName =
        typeof tableRecord?.algorithm_name === 'string'
          ? tableRecord?.algorithm_name
          : tableRecord?.algorithm_name?.[0];
              ...
              {_.includes([4, 5], text?.status) ? (
                detailData?.record?.status === 2 ? (
                  ...
                  <Button
                    type="default"
                    style={{ marginLeft: 10 }}
                    onClick={() => onResetRun(tableRecord, item.dataIndex)}
                  >
                    <IconFont type="icon-qiehuan" />
                    {intl.get('benchmarkTask.rerun')}
                  </Button>
