...
const tableColumns: ColumnsType = [
  {
    ...
    render: (text: any, record: any) => (
      <div style={{ color: COLOR[text], fontWeight: _.includes([1, 2, 3], text) ? 600 : 400 }}>{text}</div>
    )
  },
  {
...
const Leaderboard = () => {
  ...
  const onHandleTableHeader = (columns: any, body: any) => {
    ...
    return _.isEmpty(filterColumns)
      ? [...[timeColumn], ...newColumns]
      : [filterColumns, ...[timeColumn], ...newColumns];
  };
      ...
      _.forEach(resCol, column => {
        ...
        if (column?.width) {
          processedColumn.width =
            _.includes(['publish_time', 'average', 'publish_user', 'algorithm_type'], column.dataIndex) &&
            childrenCountRef?.current?.childrenCount <= 4
          ...
          <div className="ad-flex-item-full-height">
            {_.isEmpty(dataSource) ? (
              <NoDataBox
                ...
                desc={
                  !tableState?.algorithm_type && !tableState?.name
                    ? intl.get('global.noData')
