...
const Header = (props: any) => {
  ...
  const onSearch = (e: any) => {
    ...
    const data = _.filter(originData, item => {
      return (
        fuzzyMatch(searValue, _.isArray(item?.algorithm) ? item?.algorithm[0] : item?.algorithm) ||
        fuzzyMatch(searValue, item?.publish_user || '')
      );
    });
      ...
      <div className="ad-space-between">
        <div className="ad-flex">
          ...
          >
            {_.map(
              _.filter(
                _.cloneDeep([{ title: intl.get('benchmarkTask.comprehensive'), task_id: -1 }, ...saveColumns]),
                (i: any) => i.title !== 'average'
              ),
              (item: any, index) => {
                return (
                  <Radio.Button
        ...
        <div className="ad-align-center">
          ...
          <Select
            style={{ width: 200 }}
            defaultValue={0}
            value={tableState?.algorithm_type}
