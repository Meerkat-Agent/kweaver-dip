...
const DataSetFilesVersionsList = (props: DataSetFilesVersionsListProps) => {
  ...
  useEffect(() => {
    ...
    filesVersionsListStatus.order,
    filesVersionsListStatus.rule,
    filesVersionsListStatus.curVersion,
    filesVersionsListStatus.curPathKey,
    status
  ]);
  ...
  const handleOutput = async (data: any) => {
    ...
    try {
      if (Array.isArray(id) || type === 'dir') {
        // 批量下载
        const tem_name = Array.isArray(id)
          ? '【批量下载】' + _.filter(filesVersionsListStatus.dataSource, item => item.key === id[0])[0]?.name + '等'
          : name;
      ...
      render: (text: any, record: any, index: number) => {
        ...
        return (
          <div className="operation">
            ...
            <Format.Button
              type="link"
              onClick={() => handleDelete('files', { id, version: curVersion, name })}
          ...
          <BreadcrumbDir
            ...
            onLoadData={data => {
              ...
              setFilesVersionsListStatus((preState: any) => ({
                ...preState,
                curPathKey: data.key,
                breadCrumbData: preState.breadCrumbData.slice(
                  0,
                  preState.breadCrumbData.findIndex((item: any) => item.key === data.key) + 1
                ),
                loading: false
              }));
            }}
      ...
      itemDom:
        disabledOperation || _.isEmpty(selectedFiles) || isView ? null : (
          <Format.Button
            ...
            onClick={() =>
              handleDelete('files', { id: selectedFiles, version: filesVersionsListStatus.curVersion }, true)
            }
          >
    ...
    <div style={{ maxHeight: 350 }}>
      <div className="dataSet-files-versions ad-w-100 ad-h-100">
        <ADTable
          ...
          dataSource={filesVersionsListStatus.dataSource}
          rowSelection={
            disabledOperation || isView || !isViewAndDown ? undefined : { type: 'checkbox', ...rowSelection }
          }
