      ...
      const isOk = await tipModalFunc({
        ...
        content: (
          <div className={language === 'en-US' ? 'dataSet-edit-config' : ''}>
            {intl.get('dataSet.config.changeVersionTip').split('\n')[1]}
          </div>
        ),
  ...
  const items = [
    {
      label: (
        ...
        >
          <IconFont
            type={
              selectedTab === 'previewTab' || tabIsHover === 'previewTab' ? 'icon-color-moxing2' : 'icon-color-moxing1'
            }
            border
            className="ad-mr-2"
            style={{ fontSize: 16 }}
    ...
    },
    {
      label: (
        ...
        >
          <IconFont
            type={
              selectedTab === 'versionsTab' || tabIsHover === 'versionsTab'
                ? 'icon-color-zidianguanli2'
                : 'icon-color-zidianguanli1'
            }
            border
            className="ad-mr-2"
            style={{ fontSize: 16 }}
          ...
          {_.isEmpty(filesVersionsListStatus.dataSource) ? (
            <ExclamationCircleFilled
              className={'err-icon'}
              style={{ color: 'red', transform: 'translateX(-5px) translateY(-9px)', margin: 0 }}
            />
          ) : null}
        </div>
      ),
    ...
    },
    {
      label: (
        ...
        >
          <IconFont
            type={
              selectedTab === 'settingTab' || tabIsHover === 'settingTab' ? 'icon-xitongpeizhi' : 'icon-xitongpeizhi'
            }
            border
            className="ad-mr-2"
            style={{
      ...
      children: (
        <Setting
          ...
          isChange={isConfigChange}
          setIsChange={setIsConfigChange}
          ref={SettingRef}
        />
      )
    }
