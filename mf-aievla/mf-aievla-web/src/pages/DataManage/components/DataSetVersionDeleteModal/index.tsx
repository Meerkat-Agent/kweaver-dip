...
const DataSetVersionDeleteModal = (props: DataSetVersionDeleteModalProps) => {
  ...
  const handleDelete = async (data: { ds_id: string; versions: string[]; name?: string }) => {
    ...
    const isOk = await DeleteModal({
      ...
      currentDeleteType: intl.get(
        `delete.${!!data?.name || versions?.length === 1 ? 'dataSetVersion' : 'dataSetVersions'}`
      ),
        ...
        return (
          <div className="operation">
            <Format.Button
              type="link"
              onClick={() =>
                handleDelete({ ds_id: filesVersionsListStatus.root_id, versions: [version_id], name: version })
              }
