 ...
 * 获取数据集配置列表数据
 */
export const dataSetList = async (data: {
  page: number;
  size?: number;
  query: string;
  order: string;
 ...
 * 新建数据集
 */
export const dataSetCreate = async (data: {
  name: string;
  delimiter: string;
  description: string;
  color: string;
...
export const dataSetFilesEndUpload = async (data: {
  ...
  const { res }: any =
    (await studioAxios.axiosPost(`${API.dataSetFilesEndUpload}`, data, { isHideMessage: true })) || {};
  return res || true;
};
 ...
 * 分块上传文件
 */
export const dataSetFilesUploadPart = async (data: {
  upload_type: number;
  upload_id: string;
  parts: string;
  key: string;
  ...
  const { res }: any =
    (await studioAxios.axiosPost(`${API.dataSetFilesUploadPart}`, data, { isHideMessage: true })) || {};
  return res || true;
};
 ...
 * 分块上传文件完成
 */
export const dataSetFilesEndUploadPart = async (data: {
  upload_type: 0 | 1;
  part_infos: Record<any, any>;
  upload_id: string;
  key: string;
}) => {
  const { res }: any =
    (await studioAxios.axiosPost(`${API.dataSetFilesEndUploadPart}`, data, { isHideMessage: true })) || {};
  return res;
};
...
export const dataSetFilesBatchDownLoad = async (data: {
  upload_type: number;
  doc_ids: string[];
  version_id: string;
  name: string;
 ...
 * 删除数据集-指定版本的文件
 */
export const dataSetFilesDelete = async (data: {
  doc_ids: string[] | React.Key[];
  version_id: string;
  delete_type: number;
}) => {
  try {
