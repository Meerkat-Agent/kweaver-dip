...
export const downloadIndicatorFileTemplate = async () => {
  try {
    const res: any =
      (await studioAxios.axiosGet(`${API.benchmark}/metric-template`, {}, { responseType: 'blob' })) || {};
    return res || true;
  } catch (err) {
    handleCommonError(err);
    return false;
...
export const downloadAdapterFileTemplate = async (data: any) => {
  try {
    ...
    const res: any =
      (await studioAxios.axiosGet(`${API.downloadAdapterFileTemplate}?${param}`, {}, { responseType: 'blob' })) || {};
...
export const downloadInlineIndicatorFile = async (metric_id: string) => {
  try {
    const res: any =
      (await studioAxios.axiosGet(
        `${API.benchmark}/download-inline-metric`,
        { metric_id },
        { responseType: 'blob' }
      )) || {};
    if (res) {
...
export const getOutputByIndicatorIds = async (ids: string[]) => {
  try {
    const { res }: any =
      (await studioAxios.axiosGet(`${API.benchmark}/get-metric-output-map`, { metric_id_list: JSON.stringify(ids) })) ||
      {};
    return res || true;
  } catch (err) {
    handleCommonError(err);
